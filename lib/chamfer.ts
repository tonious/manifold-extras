...
import {runEdges, uniqueHalfedges, vertexPosition, curvedTriangles, halfedgeKey, halfedgesOf, flatTriangles, vertexNormal, mergedVertices, faceTriangles} from './meshutil.ts';
...
const example = () => {
  ...
  const base = setMaterial(cube([100,100,10], true).translate([0,0,5]).calculateNormals(0,minSharpAngle), baseMaterial);
  const shapes = [
    //cube([35,35,25], true).translate([0,0,25/2]),
    //cube([35,35,25], true).translate([-35/2,0,25/2+0.01]).calculateNormals(0,minSharpAngle).add(Manifold.cylinder(25,35/2,35/2).translate([0,0,0.01]).calculateNormals(0,minSharpAngle)).translate([10,0,0]),
