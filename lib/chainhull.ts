...
import type {Manifold as ManifoldType} from "manifold-3d/manifoldCAD.d.ts";
...
export const chainHull = (arr: Array<ManifoldType>, closed:boolean = false): ManifoldType => {
  const segments = arr
    .reduce(closed ? pairUpLoop : pairUpOpen, [])
    .map(pairs => Manifold.hull(pairs));
  return Manifold.union(segments);
};